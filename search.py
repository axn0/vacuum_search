...
class Node:
    ...
    def __repr__(self):  # string representation of Node object
        return "<Node {}>".format(self.state)

    def __lt__(self, node):
        return self.state < node.state

    def expand(self, problem):  # use to list the nodes to add to the frontier
...
def breadth_first_graph_search(problem):
    ...
    ***ALGORITHM - note***
    Queue to track nodes to be explored (frontier)
    Set to track nodes that have already been explored (explored)
    While there are still nodes in the frontier, explore
        check if node is the goal node, return node
        add children to queue if not explored and not in frontier
    return None, none (goal state never reached)
    """
    #node = Node(problem.initial)  # FIFO queue
    #if problem.goal_test(node.state):  # check if initial state is goal state, then we return
    #    return node, None
...
def depth_first_graph_search(problem):
    ...
    ***ALGORITHM - note***
    Stack to keep track of nodes to be explored (frontier), LIFO
